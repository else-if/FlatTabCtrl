...
public:
	...
	// initialize
	BOOL Init(HWND hWindow, bool bIsChild = false) {
		if (IsWindow(hWindow)) {
			m_hWnd = hWindow;
			BOOL bRes;
			m_bIsChild = bIsChild;
			if (m_bIsChild)
			{				
				bRes = GetWindowRect(m_hWnd, &m_pRect);
				HWND hWnd = GetParent(m_hWnd);		  
				ScreenToClient(hWnd, &((LPPOINT)&m_pRect)[0]);
				ScreenToClient(hWnd, &((LPPOINT)&m_pRect)[1]);
			}
			else
			{
				bRes = GetClientRect(m_hWnd, &m_pRect);
			}      
			if (bRes)
				return TRUE;
		}
	...
	BOOL Add(HWND hWnd, UINT uFlag) {
		...
		if (GetWindowRect(hWnd, &pRect)) {
			...
			{
				HWND hWnd = GetParent(m_hWnd);
				ScreenToClient(hWnd, &((LPPOINT)&pRect)[0]);
		...
		while (item) {
			if (item->hWnd==hWnd) { // update position
				if (GetWindowRect(hWnd, &item->pRect)) {
					...
					{
						HWND hWnd = GetParent(m_hWnd);
						ScreenToClient(hWnd, &((LPPOINT)&item->pRect)[0]);
						ScreenToClient(hWnd, &((LPPOINT)&item->pRect)[1]);
					}
					else
					{
	...
	BOOL OnSize(BOOL bRepaint=TRUE)
	{
		...
		if (m_bIsChild)
		{
			bRes = GetWindowRect(m_hWnd, &pRect);
			HWND hWnd = GetParent(m_hWnd);
			ScreenToClient(hWnd, &((LPPOINT)&pRect)[0]);
			ScreenToClient(hWnd, &((LPPOINT)&pRect)[1]);
		}
		...
		{
			bRes = GetClientRect(m_hWnd, &pRect);
		}
		if (bRes)
		{
			CDlgAncItem *item = m_pControls.next;
			...
			while (item) {
				...
				if ((item->uFlag & ANCHOR_TOP) && (item->uFlag & ANCHOR_BOTTOM)) {
					...
					if (m_bIsChild)
						item->pRect.top += pRect.top - m_pRect.top;						
				}
